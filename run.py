...
def run(model_path, input_path):
    ...
    """
    words = read_conll_single(input_path) # words: list [ { tokens: [ raw_tokens, ... ] } ... ]

    lstmModel = BiLSTM.load_model(model_path)
    data_matrix = create_data_matrix(words, lstmModel.mappings)
    tags = lstmModel.tagWords(data_matrix)['english']
